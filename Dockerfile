...
RUN apk add --no-cache git python3 py3-pip py3-gevent py3-wheel py3-scipy py3-numpy-dev python3-dev build-base jpeg-dev zlib-dev llvm17 llvm17-dev libtool supervisor autoconf automake pkgconfig
...
RUN export PATH=$PATH:/usr/lib/llvm17/bin && /app/venv/bin/pip install --no-cache-dir -r requirements.txt && \
    apk del build-base llvm17-dev python3-dev autoconf automake pkgconfig
